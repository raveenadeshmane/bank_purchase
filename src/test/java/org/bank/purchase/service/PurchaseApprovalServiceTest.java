...
class PurchaseApprovalServiceTest {
    ...
    void shouldDenyIneligibleCustomer() {
        ApprovalResult result = service.evaluatePurchaseApproval("12345678901", 300, 12);
    ...
    void shouldApproveMaxAmount() {
        ApprovalResult result = service.evaluatePurchaseApproval("12345678934", 1000, 6);
    ...
    void shouldAdjustPaymentPeriodWhenNeeded() {
        ...
        ApprovalResult result = service.evaluatePurchaseApproval("12345678923", 1500.0, 8);
        assertTrue(result.isApproved());
    ...
    void shouldAdjustPaymentPeriodWhenNeeded2() {
        // Test case where requested amount 500 is too high for 6 months
        ApprovalResult result = service.evaluatePurchaseApproval("12345678912", 500.0, 6);
        assertTrue(result.isApproved());
    ...
    void shouldHandleMinimumAmountApproval() {
        ApprovalResult result = service.evaluatePurchaseApproval("12345678923", 100, 6);
        assertTrue(result.isApproved());
    ...
    void shouldCapAtMaximumAmount() {
        ApprovalResult result = service.evaluatePurchaseApproval("12345678934", 10000, 24);
        assertTrue(result.isApproved());
